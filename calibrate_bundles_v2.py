# 文件名: calibrate_bundles_v2.py
# 描述: 路线图 A+ 的修正版实现，直接从 model.py 导入模型定义。
...
class ActivationHook:
    """简单的 hook 类，用于抓取模块的输出"""
    def __init__(self):
        self.output = None
    def __call__(self, module, module_in, module_out):
        # c_proj 的输出就是我们想要的
        self.output = module_out.detach()

def calculate_calibration_params(host_stats, bundle_stats, args):
    """根据您的 A+ 方案计算仿射校准参数 a 和 b"""
...
def fold_parameters(a, b, g, proj_weight, proj_bias):
    """将 a, b, g 折叠进 c_proj 层的权重和偏置"""
    a = a * g
    b = b * g
    new_weight = torch.diag(a) @ proj_weight
    new_bias = a * proj_bias + b if proj_bias is not None else b
    return new_weight, new_bias

# --- 主执行逻辑 ---
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="路线图 A+ (v2)：对 Bundle 移植模型进行统计校准")
    ...
    ckpt_base = torch.load(base_model_path, map_location='cpu') # Load to CPU first
    # 使用导入的 GPTConfig
    ...
    ckpt_bundle = torch.load(bundle_model_path, map_location='cpu') # Load to CPU
    ...
    for g in gating_factors:
        ...
        proj_bias = proj_layer.bias.data if proj_layer.bias is not None else None
        
        new_weight, new_bias = fold_parameters(a.to(device), b.to(device), g, proj_weight, proj_bias)
    ...
    script_content += f"echo '🚀 开始批量评估 {args.bundle_type} Bundle 的校准模型 (路线图 A+)... 🚀'\n"
    ...
    print("-" * 60)
    print(f"🎉 校准模型已全部生成！")
    print(f"   评估脚本 '{script_filename}' 也已创建。")