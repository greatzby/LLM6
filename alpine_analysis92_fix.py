...
class ALPINEMatrixExtractor:
    ...
    def extract_adjacency_matrix(self) -> np.ndarray:
        """
        提取邻接矩阵表示 W'_M - 修正版
        根据ALPINE论文：W'_M = FFN(token_emb) + token_emb
        只使用FFN，不使用attention
        """
        vocab_size = self.config.vocab_size
        W_M_prime = []
        ...
        with torch.no_grad():
            for node_i in range(vocab_size):
                ...
                # 关键修改：只通过FFN（MLP），不用完整的transformer层
                # FFN expects shape [batch, seq, n_embd]
                token_emb_expanded = token_emb.unsqueeze(0).unsqueeze(0)  # [1, 1, n_embd]
                
                # 只使用MLP部分
                ffn_out = self.model.transformer.h[0].mlp(token_emb_expanded)
                ffn_out = ffn_out.squeeze()  # [n_embd]
                ...
                logits = combined @ output_weight.T  # [vocab_size]
    ...
    def extract_reachability_matrix(self) -> np.ndarray:
        """
        提取可达矩阵表示 W'_V - 修正版
        根据ALPINE论文：W'_V = (token_emb @ W_V) + FFN(token_emb @ W_V)
        """
        vocab_size = self.config.vocab_size
        n_embd = self.config.n_embd
        W_V_prime = []
        ...
        with torch.no_grad():
            ...
            for target_node in range(vocab_size):
                ...
                # 修正：通过Value矩阵变换（注意转置）
                value_features = target_emb @ W_V.T  # [n_embd]
                
                # 关键添加：通过FFN
                value_features_expanded = value_features.unsqueeze(0).unsqueeze(0)  # [1, 1, n_embd]
                ...
                reachability_scores = combined @ output_weight.T  # [vocab_size]
