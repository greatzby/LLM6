# train_composition_multi_run.py
import os
import pickle
import argparse
import numpy as np
...
def run_single_experiment(args, embed_dim, seed):
    """运行单个实验，保持原始脚本的所有逻辑"""
    # 设置当前实验的参数
    args.n_embd = embed_dim
    args.seed = seed
    ...
    out_dir = f'out/composition_embed{embed_dim}_seed{seed}_{timestamp}'
    os.makedirs(out_dir, exist_ok=True)
    
    logger = get_logger(os.path.join(out_dir, "train.log"))
    
    print("="*60)
    print(f"Composition Training - Embed {embed_dim}, Seed {seed}")
...
def main():
    ...
    print("="*80)
    print("Will run the following experiments:")
    for i, (embed, seed) in enumerate(experiments):
        print(f"  {i+1}. Embedding dimension: {embed}, Seed: {seed}")
    print("="*80 + "\n")
    
    # 依次运行每个实验
    for i, (embed_dim, seed) in enumerate(experiments):
        print(f"\n{'#'*80}")
        print(f"EXPERIMENT {i+1}/4: Embed={embed_dim}, Seed={seed}")
        print(f"{'#'*80}\n")
        
        run_single_experiment(args, embed_dim, seed)
