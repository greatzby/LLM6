        ...
        try:
            with torch.no_grad():
                ...
                for i in range(min(num_samples, 50)):
                    # 随机选择S1和S3节点
                    s1_nodes = list(self.config.S1)
                    s3_nodes = list(self.config.S3)
                    
                    if not s1_nodes or not s3_nodes:
                        print(f"    ⚠️ S1或S3节点列表为空")
                        break
                    
                    source = np.random.choice(s1_nodes)
                    target = np.random.choice(s3_nodes)
                    
                    # 创建一个简单的输入序列 [source, 某个S2节点, target]
                    s2_nodes = list(self.config.S2)
                    if s2_nodes:
                        middle = np.random.choice(s2_nodes)
                    else:
                        middle = source  # 如果没有S2节点，使用source作为占位
