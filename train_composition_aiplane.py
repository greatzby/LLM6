# train_composition_fixed_final.py
import os
import pickle
import argparse
import numpy as np
...
def main():
    ...
    out_dir = f'out/composition_{timestamp}'
    ...
    # ===== 关键修改：自动检测训练文件 =====
    # 查找所有train_*.bin文件
    train_files = glob.glob(os.path.join(data_dir, 'train_*.bin'))
    if not train_files:
        raise FileNotFoundError(f"No train_*.bin file found in {data_dir}")
    
    # 如果有多个，选择数字最大的（通常是最新的）
    if len(train_files) > 1:
        # 提取文件名中的数字并排序
        train_files_with_nums = []
        for f in train_files:
            basename = os.path.basename(f)
            # 从 train_XX.bin 中提取 XX
            try:
                num = int(basename.replace('train_', '').replace('.bin', ''))
                train_files_with_nums.append((num, f))
            except:
                pass
        if train_files_with_nums:
            train_files_with_nums.sort(reverse=True)
            train_file = train_files_with_nums[0][1]
        else:
    ...
    for iter_num in range(args.max_iters + 1):
        ...
        if iter_num % args.checkpoint_interval == 0 and iter_num > 0:
            ...
            print(f"  Checkpoint saved: ckpt_{iter_num}.pt")
    ...
    print(f"Final S1->S3 accuracy: {results['S1->S3']['accuracy']:.2%}")
