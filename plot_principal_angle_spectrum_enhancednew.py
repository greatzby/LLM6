...
3.  **整合总体指标**: 在谱图上绘制水平虚线，代表所有余弦值的平均值 `mean(cos(θ_i))`，
    直接对应教授要求的"average cosine"指标。
...
def analyze_and_plot(iteration):
    ...
    for res in cross_seed_results:
        ax1.plot(range(1, len(res['cosines']) + 1), res['cosines'], 
                marker='.', markersize=4, linestyle='-', alpha=0.6)
        all_cs_cosines.extend(res['cosines'])
    
    # 计算并绘制平均余弦值
    if all_cs_cosines:
        mean_cos_cs = np.mean(all_cs_cosines)
        mean_angle_cs = np.arccos(np.clip(mean_cos_cs, -1, 1)) * 180 / np.pi
        ax1.axhline(y=mean_cos_cs, color='red', linestyle='--', linewidth=2.5, 
                    label=f'Mean cosine = {mean_cos_cs:.3f}\n(≈ {mean_angle_cs:.1f}° average angle)')
    ...
    for res in cross_mixture_results:
        ax2.plot(range(1, len(res['cosines']) + 1), res['cosines'], 
                marker='.', markersize=4, linestyle='-', alpha=0.7)
        all_cm_cosines.extend(res['cosines'])

    # 计算并绘制平均余弦值
    if all_cm_cosines:
        mean_cos_cm = np.mean(all_cm_cosines)
        mean_angle_cm = np.arccos(np.clip(mean_cos_cm, -1, 1)) * 180 / np.pi
        ax2.axhline(y=mean_cos_cm, color='blue', linestyle='--', linewidth=2.5, 
                    label=f'Mean cosine = {mean_cos_cm:.3f}\n(≈ {mean_angle_cm:.1f}° average angle)')
    ...
    print("="*60)
    if all_cs_cosines:
        print(f"跨种子 (Cross-Seed):")
        print(f"  - 平均余弦值: {mean_cos_cs:.4f}")
        print(f"  - 平均角度: {mean_angle_cs:.2f}°")
        print(f"  - 投影重合度 mean(cos²): {np.mean(np.square(all_cs_cosines)):.4f}")
    
    if all_cm_cosines:
        print(f"\n跨混合度 (Cross-Mixture):")
        print(f"  - 平均余弦值: {mean_cos_cm:.4f}")
        print(f"  - 平均角度: {mean_angle_cm:.2f}°")
        print(f"  - 投影重合度 mean(cos²): {np.mean(np.square(all_cm_cosines)):.4f}")
    
    if all_cs_cosines and all_cm_cosines:
        print(f"\n对比:")
        print(f"  - 混合度效应 vs 种子效应 (余弦值): {mean_cos_cm/mean_cos_cs:.2f}x")
        print(f"  - 角度差异: 跨种子 {mean_angle_cs:.1f}° vs 跨混合度 {mean_angle_cm:.1f}°")
